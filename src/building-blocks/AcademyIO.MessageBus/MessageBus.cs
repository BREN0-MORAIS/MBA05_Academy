using System;
using System.Threading.Tasks;
using EasyNetQ;
using AcademyIO.Core.Messages.Integration;
using Polly;
using RabbitMQ.Client.Exceptions;

namespace AcademyIO.MessageBus
{
    public class MessageBus : IMessageBus
    {
        private IBus _bus;
        private IAdvancedBus _advancedBus;

        private readonly string _connectionString;

        public MessageBus(string connectionString)
        {
            _connectionString = connectionString;
            TryConnect();
        }

        public bool IsConnected => _bus?.Advanced.IsConnected ?? false;
        public IAdvancedBus AdvancedBus => _bus?.Advanced;

        public void Publish<T>(T message) where T : IntegrationEvent
        {
            TryConnect();
            _bus.PubSub.Publish(message);
        }

        public async Task PublishAsync<T>(T message) where T : IntegrationEvent
        {
            TryConnect();
            await _bus.PubSub.PublishAsync(message);
        }

        public void Subscribe<T>(string subscriptionId, Action<T> onMessage) where T : class
        {
            TryConnect();
            _bus.PubSub.Subscribe(subscriptionId, onMessage);
        }

        public void SubscribeAsync<T>(string subscriptionId, Func<T, Task> onMessage) where T : class
        {
            TryConnect();
            _bus.PubSub.SubscribeAsync(subscriptionId, onMessage);
        }

        public TResponse Request<TRequest, TResponse>(TRequest request) where TRequest : IntegrationEvent
            where TResponse : ResponseMessage
        {
            TryConnect();
            return _bus.Rpc.Request<TRequest, TResponse>(request);
        }

        public async Task<TResponse> RequestAsync<TRequest, TResponse>(TRequest request)
            where TRequest : IntegrationEvent where TResponse : ResponseMessage
        {
            TryConnect();
            return await _bus.Rpc.RequestAsync<TRequest, TResponse>(request);
        }

        public IDisposable Respond<TRequest, TResponse>(Func<TRequest, TResponse> responder)
            where TRequest : IntegrationEvent where TResponse : ResponseMessage
        {
            TryConnect();
            return _bus.Rpc.Respond(responder);
        }

        public IDisposable RespondAsync<TRequest, TResponse>(Func<TRequest, Task<TResponse>> responder)
            where TRequest : IntegrationEvent where TResponse : ResponseMessage
        {
            TryConnect();
            return _bus.Rpc.Respond(responder);
        }

        private void TryConnect()
        {
            if (IsConnected) return;

            // Retry policy: exponential backoff (5 attempts)
            var retryPolicy = Policy.Handle<EasyNetQException>()
                .Or<BrokerUnreachableException>()
                .WaitAndRetry(5, retryAttempt =>
                    TimeSpan.FromSeconds(Math.Pow(2, retryAttempt)), (ex, time) =>
                    {
                        // optional: add logging here
                    });

            // Circuit breaker: break after 2 consecutive failures for 30 seconds
            var circuitBreaker = Policy.Handle<EasyNetQException>()
                .Or<BrokerUnreachableException>()
                .CircuitBreaker(2, TimeSpan.FromSeconds(30), (ex, duration) =>
                {
                    // onBreak: optional logging
                }, () =>
                {
                    // onReset: optional logging
                });

            var policyWrap = Policy.Wrap(retryPolicy, circuitBreaker);

            policyWrap.Execute(() =>
            {
                _bus = RabbitHutch.CreateBus(_connectionString);
                _advancedBus = _bus.Advanced;
                _advancedBus.Disconnected += OnDisconnect;
            });
        }

        private void OnDisconnect(object s, EventArgs e)
        {
            // Start a background reconnect loop with a backoff so the process doesn't block.
            Task.Run(async () =>
            {
                var attempt = 0;
                while (!IsConnected)
                {
                    try
                    {
                        TryConnect();
                        if (IsConnected) break;
                    }
                    catch
                    {
                        // swallow and backoff
                    }

                    attempt++;
                    var delay = TimeSpan.FromSeconds(Math.Min(60, Math.Pow(2, Math.Min(attempt, 6))));
                    await Task.Delay(delay);
                }
            });
        }

        public void Dispose()
        {
            _bus.Dispose();
        }
    }
}